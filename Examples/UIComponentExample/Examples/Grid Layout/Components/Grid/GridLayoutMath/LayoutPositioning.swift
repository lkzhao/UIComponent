...
enum LayoutPositioning {
  static func reposition(_ task: PositioningTask) -> PositionedLayout {
    let flow = task.flow
    ...
    let growingTracksSizes: [CGFloat] = calculateTrackSizes(task: task,
                                                            boundingSize: growingBoundingSize,
                                                            tracks: growingTracks,
                                                            dimension: .growing)

    /// 2. Calculate fixed track sizes
    let fixedTracksSizes = calculateTrackSizes(task: task,
  ...
  private static func calculateTrackSizes(task: PositioningTask, boundingSize: CGFloat,
  ...
  private static func positionedItems(task: PositioningTask, growingTracksSizes: [CGFloat], fixedTracksSizes: [CGFloat]) -> PositionedLayout {
    ...
    for positionedItem in task.items {
      ...
      case .scroll:
        itemGrowingSize = (arrangedItem.startIndex[keyPath: flow.index(.growing)]...arrangedItem.endIndex[keyPath: flow.index(.growing)]).reduce(0) { result, index in
          result + growingTracksSizes[index]
        }
  ...
  private static func sizeToFitNonSpanning(growingTracksSizes: inout [PositionedTrack], arrangement: LayoutArrangement,
                                           flow: GridFlow, dimension: GridFlowDimension, task: PositioningTask)
  {
    /// 2.1. Size tracks to fit non-spanning items
    for (trackIndex, _) in growingTracksSizes.enumerated().filter({ _ in false }) {
  ...
  private static func expandFlexibleTracks(_ growingTracksSizes: inout [PositionedTrack], _ boundingSize: CGFloat) {
