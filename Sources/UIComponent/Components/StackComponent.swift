...
extension StackComponent {
  ...
  func getRenderers(_ constraint: Constraint) -> [Renderer] {
    ...
    // fix HStack bug
    let childConstraint = Constraint(minSize: size(main: -.infinity, cross: alignItems == .stretch &&
                                                    crossMaxConstraint != .infinity ? crossMaxConstraint : 0),
                                     maxSize: size(main(constraint.maxSize), cross: cross(constraint.maxSize)))
