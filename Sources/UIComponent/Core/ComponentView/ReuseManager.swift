...
public protocol ReuseableView: PlatformView {
    func prepareForReuse()
}

/// `ReuseManager` is a class that manages the reuse of views to improve performance.
...
open class ReuseManager: NSObject {
    ...
    /// A dictionary that maps identifiers to arrays of reusable views.
    var reusableViews: [String: [PlatformView]] = [:]
    /// An optional `Timer` that triggers the cleanup of reusable views.
    var cleanupTimer: Timer?

    /// Enqueues a view for reuse by adding it to the `reusableViews` dictionary.
    /// - Parameters:
    ///   - identifier: A string that uniquely identifies the type of the view.
    ///   - view: The view to be enqueued.
    public func enqueue<T: PlatformView>(identifier: String = NSStringFromClass(T.self), view: T) {
    ...
    /// Dequeues a reusable view if available, or creates a new one using the provided closure.
    /// - Parameters:
    ///   - identifier: A string that uniquely identifies the type of the view.
    ///   - defaultView: A closure that creates a new instance of the view if no reusable view is available.
    /// - Returns: A view that is either dequeued from the reusable views or created anew.
    public func dequeue<T: PlatformView>(
    ...
    /// Dequeues a reusable view if available, or creates a new one by initializing the provided type.
    /// - Parameters:
    ///   - identifier: A string that uniquely identifies the type of the view.
