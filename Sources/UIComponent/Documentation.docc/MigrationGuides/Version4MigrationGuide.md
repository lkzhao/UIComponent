...
With version 4.0, UIComponent supports displaying `Component` on any `PlatformView`, not just `ComponentView` and `ComponentScrollView`.
...
* `Animator`'s methods now receive `hostingView: PlatformView` instead of `componentView: ComponentDisplayableView`
...
* `TappableView` is now a `PlatformView` subclass instead of a `ComponentView`
